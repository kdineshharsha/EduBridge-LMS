...
export default function AddCourse() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const createdCourse = courseRes.data.course;

      // 2️⃣ Loop through lessons and create each one
      for (let i = 0; i < lessons.length; i++) {
        ...
        const lessonData = {
          ...
          order: i + 1,
          course: createdCourse._id, // 🔗 link to created course
        };
      ...
      console.error("Error adding course or lessons:", error);
      toast.error("Error creating course or lessons!");
    } finally {
      setLoading(false);
    }
  }
  ...
  const handleAddLesson = async () => {
    if (!lessonTitle) {
      toast.error("Enter lesson title!");
      return;
    }

    let uploadedFileUrl = "";
    if (lessonFiles) {
      uploadedFileUrl = await mediaUpload(lessonFiles);
    }

    const newLesson = {
      title: lessonTitle,
      description: lessonDescription,
      video: lessonLink || uploadedFileUrl,
      duration: duration || 0,
    };
